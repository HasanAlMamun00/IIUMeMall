...
const Order = () => {
    ...
    const { refetch, data: bookings = [] } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`http://localhost:5000/booking/seller/${user}`)
            ...
            <table className="min-w-full table">
                <thead className="bg-[#F8FAFC]">
                    <tr>
                        ...
                        <th className=" text-sm font-normal text-left rtl:text-right text-gray-500 ">
                            <span className="text-black font-semibold text-base"> To Receive{" "} </span>
                        </th>

                        <th className=" text-sm font-normal text-left rtl:text-right text-gray-500 ">
                            <span className="text-black font-semibold text-base">  To Ship{" "} </span>
                        </th>
                ...
                <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">

                    {
                        bookings?.data?.length == 0 ?
                            <h1 className="text-center font-semibold mt-10 text-2xl">You have <strong className="underline">0</strong> order</h1>
                            :
                            bookings?.data?.map((item) => (
                                <tr key={item?.id}>
                                    ...
                                    <td className="text-sm whitespace-nowrap">
                                        {
                                            item?.status == 'booked' || item?.status == 'to ship' || item?.status == 'cash on delivery' ?
                                                <button disabled className="flex justify-center px-4 py-[10px] border-none outline-none rounded-md capitalize text-white duration-300 bg-green-300 font-semibold">
                                                    Not Yet Received
                                                </button>
                                                :
                                                <button disabled className="flex justify-center px-4 py-[10px] border-none outline-none rounded-md capitalize text-white bg-green-500 duration-300 font-semibold">
                                                    Received
                                                </button>
                                        }
                                    </td>
                                    <td className="text-sm whitespace-nowrap">
                                        {
                                            item?.status === 'booked' || item?.status == 'cash on delivery' ?
                                                <button onClick={() => handleStatus(item)} className="flex justify-center px-4 py-[10px] border-none outline-none rounded-md capitalize text-white bg-blue-500 duration-300 hover:bg-blue-600 font-semibold">
                                                    Send To Ship
                                                </button>
                                                :
                                                item?.status === 'to ship' ?
                                                    <button disabled className="flex justify-center px-4 py-[10px] border-none outline-none rounded-md capitalize text-white bg-sky-500 duration-300 hover:bg-sky-400 font-semibold">
                                                        To Ship
                                                    </button>
                                                    :
